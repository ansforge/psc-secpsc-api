...
public interface PsiApi {
	...
	/**
	 * GET /user/eims : Récupère la liste des MIE et la liste des autres ids
	 * d&#39;un utilisateur par son id national
	 *
	 * @param nationalId (required)
	 * @return Utilisateur trouvé (status code 200) or Données invalides ou absentes
	 *         (status code 400) or Utilisateur non autorisé (status code 401) or
	 ...
	 */
	@Operation(operationId = "rechercherEims", summary = "Récupère la liste des MIE et la liste des autres ids d'un utilisateur par son id national", tags = {
			"rechercher-user-controller" }, responses = {
					...
					@ApiResponse(responseCode = "500", description = "Erreur interne serveur", content = {
							@Content(mediaType = "application/json", schema = @Schema(implementation = GenericInternalServerErrorDto.class)) }) })
	@RequestMapping(method = RequestMethod.GET, value = "/user/eims", produces = { "application/json" })
	...
	/**
	 * GET /user/traitsidentite : Retourne la/les personne/s (si trouvées) au SEC
	 * PSC sur la base de ses traits d&#39;identité
	 *
	 ...
	 * @return Personne/s trouvée/s (status code 200) or Données invalides ou
	 *         absentes (status code 400) or Utilisateur non autorisé (status code
	 *         401) or Utilisateur non trouvé (status code 404) or Conflits (status
	 *         code 409) or Erreur interne serveur (status code 500)
	 */
	@Operation(operationId = "rechercherParTraitsIdentite1", summary = "Retourne la/les personne/s (si trouvées) au SEC PSC sur la base de ses traits d'identité", tags = {
			"rechercher-user-controller" }, responses = {
					...
					@ApiResponse(responseCode = "500", description = "Erreur interne serveur", content = {
							@Content(mediaType = "application/json", schema = @Schema(implementation = GenericInternalServerErrorDto.class)) }) })
	@RequestMapping(method = RequestMethod.GET, value = "/user/traitsidentite", produces = { "application/json" })
